      ...
      <div className="flex mb-8 justify-between">
        {steps.map((step) => (
          <div key={step.id} className={`flex-1 text-center text-sm font-semibold ${currentStep === step.id ? 'text-neutral-500' : currentStep > step.id ? 'text-neutral-500' : 'text-white/60'}`}>
            <div className={`mx-auto w-8 h-8 rounded-full flex items-center justify-center mb-2 ${currentStep === step.id ? 'bg-neutral-200' : currentStep > step.id ? 'bg-neutral-200' : 'bg-black'}`}>
              {step.id}
            </div>
            {step.title}
          </div>
